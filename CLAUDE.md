...
8. **End EVERY session with a three-part close-out (MANDATORY):**
   ...
   - **(c) CONTINUATION PROMPT** — a highly detailed, self-contained prompt that lets the next session thoroughly and comprehensively continue: exact next increments in order, the docs/specs/sections to read first, gates to keep green, and known constraints. Word it to **allow room for creativity and discernment** — the next session should be free to spot and propose improvements along the way, not just execute mechanically. Deliver it TWICE: (1) written into RESUME_HERE's NEXT STEPS, and (2) **pasted verbatim as a fenced copy-paste block at the very END of the final chat report**, worded to work as the FIRST message of a brand-new stateless conversation (no reference to "this session" without the context needed to resolve it). The founder copies this block straight into a fresh chat — it must stand entirely on its own.
