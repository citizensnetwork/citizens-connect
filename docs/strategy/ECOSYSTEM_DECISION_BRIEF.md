...
| **4d ▶ IN PROGRESS — increments 1–7 SHIPPED (2026-07-05); wiring-spec item (a) COMPLETE + Phase C items 13 (de-scattering) + 14 (dormancy guardian) LIVE** | **Vision demo→live wiring** per the §8 critical path of [`VISION_BACKEND_WIRING_SPEC`](../VISION_BACKEND_WIRING_SPEC.md). **Increment 1 (migs 147+148, RESUME §3Q):** `vision.spaces` + daily snapshot cron (jobid 11) + `reach_per_org`/`engagement_per_org`/`calendar_growth`/`retention_rate` + MV cron (jobid 12); app `GET /api/metrics/connect` + `live.jsx` overlay (home pulse/observations + Analytics RGRE live). **Increment 2 (migs 149+150, RESUME §3R):** **mig 149** = advisory evaluation engine `evaluate_advisory_rules()` (hardcoded metric-slug→query map, cooldown-aware, small-org noise guards) + 6-hourly cron + `advisory_fill()` + 5 universal org-type-agnostic templates/rules; **mig 150** = `activity_funnel()` + `broadcast_effectiveness()` (org-level). App: Advisories screen + **home banner** wired to `/api/advisory`; Analytics **Funnel + Broadcast** tabs flipped demo→live. **Increment 3 (migs 151+152, RESUME §3S):** space-level tranche — **mig 151** = `reach_per_space()` + `engagement_per_space()` (per-space RGRE, one row per space, distinct persons) + `set_category_space()` (is_org_admin mapping writer); **mig 152** = `get_category_spaces()` (is_org_member mapping reader for UI pre-fill). App: `GET/POST /api/spaces` + `PUT/DELETE /api/spaces/[id]` + `GET/PUT /api/spaces/mappings`; `live.jsx` overlays the Spaces **directory**; `ConfigureSpaces` gains real CRUD + the category→space mapping UI. **Increment 4 (mig 153, RESUME §3T):** the editable-collection CRUD frontend — **Objectives** (`/api/goals` + `/api/metrics/alignment` ring), **Projects** (`/api/projects`), **Vision-statements** (`/api/vision`), and **Activities** (`/api/activities` + NEW `GET /api/metrics/activities` wrapping **mig 153** `activity_metrics(org)` = per-activity Reach/Engagement/Rating for CLAIMED activities) — each with live optimistic add/delete + revert, demo local-only, honest em-dash degradation. **Increment 5 (mig 154, RESUME §3U):** the **Team** screen (§3.11) — **mig 154** `org_members(org)`, a SECDEF roster reader joining `public.profiles` for display-safe identity only (never email/PII, R2); `GET /api/orgs/[orgId]/members` now returns **named** members, the Settings Team card goes live (org_admin role-change PATCH / remove DELETE, optimistic + revert, self-mutation hidden), invite POST stays an MVP stub. **This completes wiring-spec item (a).** **Increment 6 (mig 155, RESUME §3V):** Timeline Map (item b) SKIPPED — `NEXT_PUBLIC_MAPTILER_KEY` still empty in the built `config.js`, so per the roadmap the placeholder stays (never a broken map); proceeded to **Phase C item 13** = **mig 155** `cross_pollination(org, from, to)`, the platform's **de-scattering** measure (§4.2): across an org's engaged audience (reuses `org_active_persons`), how many citizens connected in the window with a `role='contributor'` org they'd NEVER engaged with before (rsvps+follows; first-ever touch inside the window). num+den per §5; org-local window default 90d, span capped at 366 (scale guard); NO PII (profiles join reads role+timezone only, deployed def verified email-free). App: NEW `GET /api/metrics/cross-pollination` (best-effort, 42501→403) → `live.jsx` overlays a home **crossPollination observation** (honest neutral variant when the audience found no new org), demo showcases it (obs-5). Advisor fingerprint diff = **exactly +1** (`cross_pollination`, the vision authenticated-SECDEF class is now THIRTEEN — by design); the true LIVE advisor absolute is **91 WARN / 3 INFO** (the running per-mig tally 72→…→84 counted only the newly-added vision wiring readers — future increments gate on the fingerprint diff, not the number). **Increment 7 (mig 156, RESUME §3W):** Timeline Map (item b) SKIPPED again (key still empty); proceeded to **Phase C item 14** = **mig 156** `dormancy_watch(org, threshold_days, lookback_days)`, the de-scattering **GUARDIAN** (§4.5) — the recency complement to cross-pollination. Across an org's engaged audience (`org_active_persons`, 180d lookback), which OTHER `role='contributor'` orgs in the ORBIT (audience rsvps+follows, the mig-155 `inwin` pattern) have gone quiet — `last_activity = GREATEST(latest published event, latest non-deleted broadcast)` older than a threshold (default 60d); orgs that never spoke are excluded ("previously active"). num+den per §5; **NO PII** (reads role only; dormant orgs' PUBLIC names resolved app-side via `/api/v1/profiles/{id}`). **Founder decision:** subject = "orbit contributors" (not the org's own citizens, not partnership-only). App: NEW `GET /api/metrics/dormancy` (days = threshold; `connectApi.getProfile` name resolution, best-effort) → `live.jsx` synthesizes a home/Advisories **dormancy advisory** (neutral/`bodyNoNames`/suppressed variants); `data.jsx` +`bodyNoNames` template variant. Fingerprint diff = **exactly +1** (`dormancy_watch`, the vision authenticated-SECDEF class is now FOURTEEN — by design), `anon_security_definer` unchanged at 10; true LIVE absolute now **92 WARN / 3 INFO**. Prod smoke: orbit 2 / dormant 2 / 100% (2/2) / max 72d; thr 90d→0 dormant (neutral); non-member 42501; net-zero. Gates each increment: 0 ERROR · 0 new-class findings · rolled-up prod smoke · tsc 0 · eslint 0 · vitest (674→**737**) · next build · frontend build · in-browser. citizens-vision `40e56af` → `7d5fd72` → `f80d038` → `9599d6f` → `2c0bc2a` → `4b9feed` → `f9fdb23` → `3c77959`. **Remaining:** Timeline Map (MapLibre + `NEXT_PUBLIC_MAPTILER_KEY` — founder must set the key), Phase C item 15 (network graph §4.3), Phase D (exports/partnerships/scheduled reports); the member **invite** path stays stubbed until a Supabase Admin/invite flow lands. Runtime gated on founder's `vision` PostgREST exposure + Vercel env. | spec §8 · RESUME §3Q+§3R+§3S+§3T+§3U+§3V+§3W |
| **5 ✅ EXECUTED (2026-07-03) — [PR #23](https://github.com/citizensnetwork/citizens-wear/pull/23) pending founder merge** | **Monorepo consolidation** delivered exactly per D3/§4: `citizens-wear` grown into the `citizens` monorepo (local clone at `Citizen Network/citizens`, branch `step5-monorepo-lift`); Connect (`ac1345c`) + Vision (`3602a86`) lifted via `git filter-repo --to-subdirectory-filter` with full history; `apps/web`→`apps/wear` + `@citizens-wear/*`→`@citizens/*` rename; **`supabase/` hoisted to the root** (head 146); vendored frontend-build copies → `workspace:*`; one root `pnpm-lock.yaml`; monorepo README/CLAUDE.md; CI reconciled; merged-tree `pnpm audit` 34→1-low (Next 15.5.18/16.2.6, turbo 2.9.14, vitest 3.2.6 + scoped overrides). Gates at HEAD: lint 10/10 · typecheck 10/10 · test 9/9 · build · format · audit. Anchor tags `{connect,vision,wear}-pre-monorepo` in the original repos. **Remaining = founder dashboard cutover:** merge PR → rename repo → Vercel ×3 (Root Directory `apps/*`, `npx turbo-ignore`) → archive old repos (RESUME §3P). | D3 · §4 |
