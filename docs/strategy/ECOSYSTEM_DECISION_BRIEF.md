...
| **4d ▶ IN PROGRESS — increments 1+2 SHIPPED (2026-07-03)** | **Vision demo→live wiring** per the §8 critical path of [`VISION_BACKEND_WIRING_SPEC`](../VISION_BACKEND_WIRING_SPEC.md). **Increment 1 (migs 147+148, RESUME §3Q):** `vision.spaces` + daily snapshot cron (jobid 11) + `reach_per_org`/`engagement_per_org`/`calendar_growth`/`retention_rate` + MV cron (jobid 12); app `GET /api/metrics/connect` + `live.jsx` overlay (home pulse/observations + Analytics RGRE live). **Increment 2 (migs 149+150, RESUME §3R):** **mig 149** = advisory evaluation engine `evaluate_advisory_rules()` (hardcoded metric-slug→query map, cooldown-aware, small-org noise guards) + 6-hourly cron + `advisory_fill()` + 5 universal org-type-agnostic templates/rules; **mig 150** = `activity_funnel()` + `broadcast_effectiveness()` (org-level). App: Advisories screen + **home banner** wired to `/api/advisory` with a dismiss round-trip (pre-rendered server-side); Analytics **Funnel + Broadcast** tabs flipped demo→live. New advisor baseline **78 WARN / 3 INFO** (6 vision reader WARNs, all by design). Gates each increment: 0 ERROR · 0 new-class findings · rolled-up prod smoke · tsc 0 · eslint 0 · vitest 674/674 · next build · frontend build · in-browser. citizens-vision `40e56af` → `7d5fd72`. **Remaining:** space-level reach/engagement fns (§3.5b) + Spaces CRUD (the Spaces directory needs §3.5b metrics first to not regress below demo), Activities/Goals/Projects/Vision-statements/Team CRUD wiring, Timeline Map (MapLibre + key), Phase C items 13–15, Phase D. Runtime gated on founder's `vision` PostgREST exposure + Vercel env. | spec §8 · RESUME §3Q+§3R |
