...
| **4d ▶ IN PROGRESS — increments 1+2+3 SHIPPED (2026-07-04)** | **Vision demo→live wiring** per the §8 critical path of [`VISION_BACKEND_WIRING_SPEC`](../VISION_BACKEND_WIRING_SPEC.md). **Increment 1 (migs 147+148, RESUME §3Q):** `vision.spaces` + daily snapshot cron (jobid 11) + `reach_per_org`/`engagement_per_org`/`calendar_growth`/`retention_rate` + MV cron (jobid 12); app `GET /api/metrics/connect` + `live.jsx` overlay (home pulse/observations + Analytics RGRE live). **Increment 2 (migs 149+150, RESUME §3R):** **mig 149** = advisory evaluation engine `evaluate_advisory_rules()` (hardcoded metric-slug→query map, cooldown-aware, small-org noise guards) + 6-hourly cron + `advisory_fill()` + 5 universal org-type-agnostic templates/rules; **mig 150** = `activity_funnel()` + `broadcast_effectiveness()` (org-level). App: Advisories screen + **home banner** wired to `/api/advisory`; Analytics **Funnel + Broadcast** tabs flipped demo→live. **Increment 3 (migs 151+152, RESUME §3S):** space-level tranche — **mig 151** = `reach_per_space()` + `engagement_per_space()` (per-space RGRE, one row per space, distinct persons) + `set_category_space()` (is_org_admin mapping writer); **mig 152** = `get_category_spaces()` (is_org_member mapping reader for UI pre-fill). App: `GET/POST /api/spaces` + `PUT/DELETE /api/spaces/[id]` + `GET/PUT /api/spaces/mappings`; `live.jsx` overlays the Spaces **directory** (per-space reach/people/activities/trend); `ConfigureSpaces` gains real CRUD + the category→space mapping UI + a live empty-state. New advisor baseline **82 WARN / 3 INFO** (the vision authenticated-SECDEF reader/writer WARNs are all by design). Gates each increment: 0 ERROR · 0 new-class findings · rolled-up prod smoke · tsc 0 · eslint 0 · vitest (674→703) · next build · frontend build · in-browser. citizens-vision `40e56af` → `7d5fd72` → `f80d038`. **Remaining:** Activities/Goals/Projects/Vision-statements/Team CRUD wiring (handlers exist), Timeline Map (MapLibre + key), Phase C items 13–15, Phase D. Runtime gated on founder's `vision` PostgREST exposure + Vercel env. | spec §8 · RESUME §3Q+§3R+§3S |
