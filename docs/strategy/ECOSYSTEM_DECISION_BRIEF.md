...
| **4d ▶ IN PROGRESS — increment 1 SHIPPED (2026-07-03)** | **Vision demo→live wiring** begun per the §8 critical path of [`VISION_BACKEND_WIRING_SPEC`](../VISION_BACKEND_WIRING_SPEC.md). **DB (Connect migs 147+148, applied, advisors 0 ERROR):** `vision.spaces` (Phase A.1) + the intelligence engine's first tranche — **daily snapshot cron** (jobid 11, hourly, org-tz aware, whole-org + per-space rows — unblocks Growth/Retention), `reach_per_org`/`engagement_per_org`/`calendar_growth`/`retention_rate` (SECDEF, membership-gated, num+den convention) + daily MV refresh cron (jobid 12). New advisor baseline **76 WARN / 3 INFO** (+4 = the gated authenticated-callable readers, by design). **App (citizens-vision `40e56af`):** `GET /api/metrics/connect` wraps the four readers + snapshot series; NEW `live.jsx` overlay drops the RPC rows into the narrative-engine data slots — home pulse/observations + Analytics Reach/Growth/Retention/Engagement go live with graceful demo fallback; Shell gains a Live/Demo badge. Gates: tsc 0 · eslint 0 · vitest 672/672 (+8) · next build · frontend build · verified in-browser. **Remaining:** advisory engine (§3.7c) + funnel/broadcast fns (mig 149+), Spaces/Activities/Goals/Team CRUD wiring, Timeline Map (MapLibre + key), Phases C/D. Runtime gated on founder's `vision` PostgREST exposure + Vercel env. | spec §8 · RESUME §3Q |
