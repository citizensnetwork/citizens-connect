...
export default function EventsView({
  ...
  return (
    <div className="relative h-dvh w-full overflow-hidden bg-(--surface)">
        ...
        <div className="mx-auto flex w-full max-w-5xl flex-col gap-2">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="pointer-events-auto flex items-center gap-2">
              ...
              >
                {view === "map" ? (
                  <svg viewBox="0 0 24 24" fill="none" stroke="#D4AF37" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                ) : (
                  <svg viewBox="0 0 24 24" fill="none" stroke="#D4AF37" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6"/><line x1="8" y1="2" x2="8" y2="18"/><line x1="16" y1="6" x2="16" y2="22"/></svg>
                )}
              </button>
            </div>
          </div>
      ...
      {/* ── Featured panel open button (shown when panel is closed) ─── */}
      ...
      >
        {/* ── Title bar — 100% opaque, no star, centered title, drag-handle close ── */}
        <div className="flex-none rounded-t-2xl border border-b-0 border-(--gold)/30 bg-white">
          {/* Drag handle pill — press or swipe-down to collapse */}
          <FeaturedDragHandle onClose={() => setFeaturedOpen(false)} />
          {/* Centered title */}
          <div className="pb-2.5 text-center">
            <h2 className="text-[11px] font-bold uppercase tracking-[0.18em] text-black/75">
              Featured
            </h2>
          </div>
        </div>

        {/* ── Content — ~30% opacity background so map shows through ── */}
        <div className="flex-1 overflow-y-auto rounded-b-2xl border border-t-0 border-(--gold)/20 bg-white/30 backdrop-blur-md">
          <FeaturedPanel
            onSelectEvent={handleSelectEvent}
            onSelectPlace={handleSelectPlace}
...
/* ── Drag-handle component for the Featured panel ─────────────────── */

function FeaturedDragHandle({ onClose }: { onClose: () => void }) {
  ...
  return (
    <button
      ...
      className="group flex w-full cursor-pointer justify-center pb-2 pt-3"
      aria-label="Collapse featured panel"
    >
      <span
